...
const grammarDef = "formula -> (\"!\" formula) | (formula \"\\\\/\" formula) | atom | hole\natom -> \"a\" | \"b\"\nhole -> \"?\"\n"
let rules = grammarDef.trim().split("\n")
const ruleMap = new Map();
const nonterminals = rules.map(r => {
    let rule = r.split(" -> ");
    let left = rule[0]
    let right = rule[1].split(" | ").map(v => v.trim())
    ruleMap.set(left, right)
    return { "ruleLeft": left, ruleRight: right }
});
console.log(ruleMap);
...
},{"../grammarDef/grammar.js":2,"nearley":3}],2:[function(require,module,exports){
